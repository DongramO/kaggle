...
def _auto_commit_if_best(current_auc: float, run_label: str) -> None:
    ...
    try:
        subprocess.run(
            ["git", "add",
             f"{project_name}/auc_history.json",
             f"{project_name}/fi_history.json",
             f"{project_name}/best_hyperparameters.json",
             f"{project_name}/main.py",
             f"{project_name}/modeling.py",
             f"{project_name}/prepare_data.py",
             f"{project_name}/history.py",
             f"{project_name}/run_optuna.py",
             ],
...
def run_single_model(model_type: str, data_dir: str | Path = "data", target_col: str = "target"):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("catboost", "lightgbm", "xgboost"):
        run_single_model(sys.argv[1], data_dir="data", target_col=TARGET_COL)
    else:
        main(data_dir="data", target_col=TARGET_COL, id_col=ID_COL)
