...
def train_model(X: pd.DataFrame, y: pd.Series, model_type: str, best_params: dict):
    ...
    if model_type == "catboost":
        ...
        model = xgb.XGBClassifier(**params, monotone_constraints=monotone_constraints)
    else:
...
class _FeatureTokenizer(nn.Module):
    ...
    def __init__(self, n_features: int, d_token: int):
        super().__init__()
        # 피처마다 독립적인 가중치/편향 — 피처 간 상호작용을 Transformer가 학습
...
class _FTTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.transformer(self.tokenizer(x))  # (B, F+1, d_token)
        return self.head(out[:, 0]).squeeze(1)     # CLS 토큰으로 예측
...
def train_ft_transformer(
    ...
    # Flash Attention 비활성화 (일부 GPU에서 invalid configuration 에러 방지)
...
def analyze_mlp_permutation_importance(
    ...
    auc_scorer = lambda est, X, y: roc_auc_score(y, est.predict_proba(X)[:, 1])
    result = permutation_importance(mlp, X_val_scaled, y_val, n_repeats=10, random_state=42, scoring=auc_scorer)
    mlp_fi = pd.DataFrame({
